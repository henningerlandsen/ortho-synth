...
void updateOscFreq() {
  const float low_octave = 55;
  float base_freq = low_octave;
  for (int i=0; i<osc[0].octave; i++) {
    base_freq += base_freq;
  }
  wave1.frequency(base_freq + osc[0].tune);

  base_freq = low_octave;
  for (int i=0; i<osc[1].octave; i++) {
    base_freq += base_freq;
  }
  wave2.frequency(base_freq + osc[1].tune);
}
